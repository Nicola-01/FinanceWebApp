...
class JwtServiceTest {
  ...
  private final String SECRET_KEY =
      "404E635266556A586E3272357538782F413F4428472B4B6250645367566B5970"; // 256-bit hex key
  ...
  void setUp() {
    ...
    ReflectionTestUtils.setField(
        jwtService, "JWT_REFRESH_EXPIRATION", 1000 * 60 * 60 * 24 * 30L); // 30 days
  ...
  void generateToken_ValidUser_GeneratesValidTokenAndCanExtractUsernameAndVersion() {
    ...
    assertEquals(
        "customValue",
        jwtService.extractClaim(token, claims -> claims.get("customClaim", String.class)));
  }
  ...
  void isTokenValid_InvalidTokenVersion_ReturnsFalse() {
    ...
    // Simulating that the token version in the DB has changed (e.g. after logout-all or password
    // change)
  ...
  void generateToken_NonUserDetails_DoesNotIncludeVersionClaim() {
    org.springframework.security.core.userdetails.User nonUser =
        (org.springframework.security.core.userdetails.User)
            org.springframework.security.core.userdetails.User.withUsername("testuser@example.com")
                .password("password")
