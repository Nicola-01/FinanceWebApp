...
public class WalletAccessRepositoryTest {
  ...
  void findAllByUserId_ShouldReturnAccessesWithEagerWallet() {
    ...
    // In a true integration test we'd check hibernate statistics, but here we can just verify it
    // returns correctly
  ...
  void existsByWalletIdAndUserIdAndStatusIn_ShouldReturnTrueIfStatusMatches() {
    boolean exists =
        walletAccessRepository.existsByWalletIdAndUserIdAndStatusIn(
            testWallet.getId(),
            testUser.getId(),
            new WalletAccess.InvitationStatus[] {
              WalletAccess.InvitationStatus.ACCEPTED, WalletAccess.InvitationStatus.PENDING
            });
    assertTrue(exists);
  }
  ...
  void existsByWalletIdAndUserIdAndStatusInAndUpdatedAtAfter_ShouldReturnTrue() {
    boolean exists =
        walletAccessRepository.existsByWalletIdAndUserIdAndStatusInAndUpdatedAtAfter(
            testWallet.getId(),
            testUser.getId(),
  ...
  void findByUserIdAndWalletId_ShouldReturnAccess() {
    Optional<WalletAccess> found =
        walletAccessRepository.findByUserIdAndWalletId(testUser.getId(), testWallet.getId());
