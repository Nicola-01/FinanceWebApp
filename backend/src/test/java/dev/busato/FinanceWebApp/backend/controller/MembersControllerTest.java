...
class MembersControllerTest extends BaseWebMvcTest {
    ...
    void getMembers_ShouldReturn200() throws Exception {
        ...
        when(memberService.getMembers(eq(walletId), any(UUID.class)))
                .thenReturn(List.of(mockResponse));

        mockMvc.perform(get("/api/invitations/{walletID}", walletId))
    ...
    void inviteMember_ShouldReturn200() throws Exception {
        ...
        mockMvc.perform(post("/api/invitations/{walletID}", walletId)
    ...
    void updateMemberRole_ShouldReturn200() throws Exception {
        ...
        when(memberService.updateMemberRole(eq(walletId), eq(memberId), any(MemberRequest.class), any(UUID.class)))
                .thenReturn(mockResponse);

        mockMvc.perform(put("/api/invitations/{walletID}/{memberID}", walletId, memberId)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.username").value("testuser"));
    ...
    void removeMember_ShouldReturn204() throws Exception {
        ...
        mockMvc.perform(delete("/api/invitations/{walletID}/{memberID}", walletId, memberId))
    ...
    void getInvites_ShouldReturn200() throws Exception {
        WalletInviteResponse mockResponse = WalletInviteResponse.builder().wallet(dev.busato.FinanceWebApp.backend.dto.WalletResponse.builder().name("Shared Wallet").build()).build();

        when(memberService.getInvites(any(User.class)))
                .thenReturn(List.of(mockResponse));
    ...
    void acceptInvite_ShouldReturn204() throws Exception {
        ...
        mockMvc.perform(post("/api/invitations/{walletID}/accept", walletId))
                .andExpect(status().isNoContent());

        verify(memberService).setStatus(any(UUID.class), eq(walletId), eq(WalletAccess.InvitationStatus.ACCEPTED));
    }
    ...
    void rejectInvite_ShouldReturn204() throws Exception {
        ...
        mockMvc.perform(post("/api/invitations/{walletID}/reject", walletId))
                .andExpect(status().isNoContent());

        verify(memberService).setStatus(any(UUID.class), eq(walletId), eq(WalletAccess.InvitationStatus.REJECTED));
    }
}
