...
class OAuthAuthCodeStoreTest {
  ...
  void generateAndStore_ValidInput_ReturnsCodeAndStoresEntry() {
    String code =
        store.generateAndStore("challenge1", "plainToken1", "client1", "http://redirect", "read");
    ...
    assertThrows(
        ...
        () ->
            store.generateAndStore(
                "challenge_dup", "token2", "client2", "http://redirect2", "write"));
  }

  @Test
  void consume_ValidCode_ReturnsEntryAndRemovesIt() {
    String code =
        store.generateAndStore("challenge2", "plainToken2", "client2", "http://redirect", "read");
  ...
  void consume_ExpiredCode_ReturnsEmpty() throws Exception {
    ...
    String code =
        store.generateAndStore("challenge_expired", "token3", "client3", "http://r", "read");
    ...
    var internalMap =
        (java.util.concurrent.ConcurrentHashMap<String, AuthCodeEntry>) storeField.get(store);

    AuthCodeEntry expiredEntry =
  ...
  void cleanup_RemovesExpiredEntriesFromBothMaps() throws Exception {
    ...
    @SuppressWarnings("unchecked")
    var usedChallenges =
        (java.util.concurrent.ConcurrentHashMap<String, Instant>) usedChallengesField.get(store);
