...
class RegistrationsRepositoryTest {
  ...
  void deleteExpiredInvitations_ShouldDeleteOnlyExpired() {
    ...
    // Assert
    Optional<Registrations> foundExpired =
        registrationsRepository.findByEmailIgnoreCase("expired@example.com");
    assertTrue(foundExpired.isEmpty());
    Optional<Registrations> foundValid =
        registrationsRepository.findByEmailIgnoreCase("valid@example.com");
