 ...
 *
 * <p>This filter runs BEFORE {@link JwtAuthenticationFilter} in the filter chain. It intercepts
 * {@code Authorization: Bearer fin_pat_...} headers and, if valid, sets the SecurityContext with
 * the token owner's identity.
 *
 * <p>If the Bearer token does NOT start with {@code fin_pat_}, this filter is a no-op and the
 * request falls through to the JWT filter.
 */
@Component
