...
public class RegisterService {
  ...
  public void requestPasswordReset(String email) {
    ...
    try {
      sendEmailService.sendForgotPasswordEmail(
          user.getEmail(), resetUrl, resetRequest.getExpiresAt());
    } catch (Exception e) {
  ...
  public void resetPassword(String token, ResetPasswordRequest request) {
    ...
    User user =
        userRepository
            .findByEmailIgnoreCase(record.getEmail())
            .orElseThrow(() -> new IllegalArgumentException("User not found."));

    user.setPassword(passwordEncoder.encode(pw));
    user.setPasswordMustChange(false);
    user.setTokenVersion(
        user.getTokenVersion() + 1); // Increments token version to invalidate old sessions
