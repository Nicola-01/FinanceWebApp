 ...
 *
 * R2 is S3-compatible: we just point the endpoint at the R2 account URL.
 * The client is built lazily (only when r2.enabled=true) so local dev
 * without any R2 credentials still works.
 */
@Slf4j
@Service
public class R2StorageService {
    ...
    private void init() {
        ...
        if (accessKey.isBlank() || secretKey.isBlank() || endpoint.isBlank() || bucket.isBlank()) {
            log.warn("[R2] One or more R2 config values are empty – R2 integration will fail at runtime.");
        }
        s3 = S3Client.builder()
    ...
    /**
     * List all backup objects in the bucket, sorted newest-first.
     * Returns a list of [key, lastModified, sizeBytes] records.
     */
    ...
    private void requireEnabled() {
        if (!enabled || s3 == null) {
            throw new IllegalStateException("R2 integration is disabled – set r2.enabled=true and configure credentials.");
        }
    }
