 ...
 *
 * <p>backup → pg_dump | gzip | openssl enc → local .sql.gz.enc → upload to R2 restore → download
 * from R2 (or local) → openssl dec | gzip -d → psql download→ download from R2 (or local) → decrypt
 * → return bytes upload → save uploaded bytes locally → push to R2 list → delegate to
 * R2StorageService.listBackups()
 */
@Slf4j
@Service
...
public class BackupService {
  ...
  /**
   * 1. pg_dump → gzip → openssl enc → .sql.gz.enc (local) 2. Upload to R2 (if enabled) 3. Cleanup
   * local file
   */
  ...
  /**
   * List available backups. - R2 enabled → list from R2 bucket - R2 disabled → list local
   * .sql.gz.enc files
   */
  ...
  /**
   * Download (decrypt) the backup identified by the given R2 key / local filename. If R2 is
   * enabled, the .enc file is first fetched from R2 into a temp path.
   */
  ...
  /**
   * Restore the database from the backup identified by the given key. Downloads from R2 if enabled,
   * otherwise uses local file.
   */
  ...
  /**
   * pg_dump directly via TCP (no docker exec required). PGPASSWORD is passed via environment
   * variable, stdout → sqlFile. Stderr is captured separately so errors are visible in logs.
   */
  private void runDockerPgDump(Path sqlFile) throws IOException, InterruptedException {
    ProcessBuilder pb =
        new ProcessBuilder(
            "pg_dump", "-h", dbHost, "-p", dbPort, "-U", dbUser, "--clean", "--if-exists", dbName);
  ...
  private void compressAndEncrypt(Path sqlFile, Path encFile)
    ...
    ProcessBuilder sslPb =
        new ProcessBuilder(
            ...
            "pass:" + encryptionPass,
            "-out",
            encFile.toAbsolutePath().toString());
  ...
  /**
   * psql restore directly via TCP (no docker exec required). stdin ← sqlFile, stderr captured for
   * logging.
   */
  private void runDockerPsqlRestore(Path sqlFile) throws IOException, InterruptedException {
    ProcessBuilder pb =
        new ProcessBuilder(
            "psql", "-h", dbHost, "-p", dbPort, "-U", dbUser, "-d", dbName, "--quiet");
    pb.environment().put("PGPASSWORD", dbPass);
  ...
  private void runProcess(ProcessBuilder pb, String label)
      throws IOException, InterruptedException {
