...
public class AdminDatabaseController {
    ...
    public ResponseEntity<String> uploadBackup(@RequestParam("file") MultipartFile file) throws Exception {
        if (file.isEmpty()) return ResponseEntity.badRequest().body("No file provided.");
        log.info("[AdminDatabaseController] Upload received – name={} size={}", file.getOriginalFilename(), file.getSize());
