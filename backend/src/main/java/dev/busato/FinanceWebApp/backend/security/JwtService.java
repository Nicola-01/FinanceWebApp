...
public class JwtService {
  ...
  /**
   * Genera l'access token con dati extra (ruolo, ID, tokenVersion). La tokenVersion viene sempre
  ...
  /**
   * Valida il token: username + scadenza + tokenVersion. Se la versione nel token non corrisponde a
   * quella nel DB, il token è invalido.
   */
  ...
  /**
   * Finestra di rinnovo: ultimi 7 giorni di vita del refresh token. Se il token scade entro questo
   * periodo, va ruotato con uno nuovo.
   */
  private static final long RENEWAL_WINDOW_MS = 7L * 24 * 60 * 60 * 1000; // 7 giorni

  /**
   * Controlla se il refresh token è nella finestra di rinnovo (ultimi 7 giorni). Se true, il token
   * dovrebbe essere ruotato con uno nuovo.
   */
