...
export const TagBadge = ({tag, showParent = true, forceShowParent = false, compact = false, onClick}: {
    tag: Tag | any,
    showParent?: boolean,
    ...
    return (
        <span className={`inline-flex items-center gap-1.5 ${compact ? 'min-w-0 w-full' : ''}`} onClick={onClick}>
            <span
            ...
            >
                <FontAwesomeIcon icon={ICONS[tag.icon as IconKey] || faTags} className={`${compact ? 'text-[8px] sm:text-[10px]' : 'text-[10px]'} opacity-70 shrink-0`}/>
