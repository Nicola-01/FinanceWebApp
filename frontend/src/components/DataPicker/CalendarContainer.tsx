...
export default function CalendarContainer({
  ...
  const handlePrevDecade = () => {
    setDirection("prev");
    setCurrentDate((prev) => subYears(prev, 10));
  };
      ...
      {view === "calendar" && (
        <>
          <div className="flex items-center justify-between mb-4 px-2">
            <button onClick={handlePrevMonth} className={btnNav}>
            ...
            <button onClick={handleNextMonth} className={btnNav}>
          ...
          <div className="grid grid-cols-7 mb-2">
            {weekDays.map((day) => (
              <div
                key={day}
                className={`text-center text-xs font-medium ${textMuted}`}
              >
                {day}
              </div>
            ))}
          ...
          <div
            key={currentDate.toISOString() + "-grid"}
            className={`flex-1 ${direction === "next" ? "anim-next" : "anim-prev"}`}
          >
            <MonthGrid
