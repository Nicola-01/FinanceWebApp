...
export const TagFilter: React.FC<TagFilterProps> = ({ tags, selectedTags, color = 'var(--color-app-green)', onChange }) => {
    ...
    const getChildren = (parentName: string) => tags.filter(t => t.parentName === parentName);

    const getFamily = (parentTag: Tag) => [parentTag, ...getChildren(parentTag.name)];
    ...
    const toggleParent = (parentTag: Tag) => {
        ...
        const isFullyChecked = familyNames.every(name => selectedTags.includes(name));
    ...
    const getParentState = (parentTag: Tag): 'checked' | 'unchecked' | 'indeterminate' => {
        const familyNames = getFamily(parentTag).map(t => t.name);
        const checkedCount = familyNames.filter(name => selectedTags.includes(name)).length;
                                            ...
                                            {children.map(childTag => {
                                                ...
                                                return (
                                                    <TagFilterRow
                                                        ...
                                                        onToggleSelection={() => toggleSingle(childTag.name)}
                                                        color={color}
                                                    />
                                                );
                                            })}
