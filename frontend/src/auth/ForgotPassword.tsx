...
import {
    faEnvelope, faArrowLeft, faSpinner,
...
const ForgotPassword: React.FC = () => {
    ...
    const handleSubmit = useCallback(async (e?: React.FormEvent) => {
        ...
        try {
            ...
            setShake(true);
            setTimeout(() => setShake(false), 500);
            triggerToast(err.response?.data?.title || "Failed to send reset email.", false);
        } finally {
            setLoading(false);
        }
    }, [email]);
    ...
    const handleResend = useCallback(async () => {
        ...
        try {
            ...
            triggerToast(err.response?.data?.title || "Failed to resend reset email.", false);
        } finally {
            setLoading(false);
        }
    }, [email, cooldown]);
                        ...
                        <form onSubmit={handleSubmit} className="flex flex-col w-full" noValidate>
                            ...
                            >
                                {loading ? (
                                    ...
                                    <><FontAwesomeIcon icon={faPaperPlane} className="mr-2" />SEND RESET LINK</>
                                )}
                            </button>
                        </form>
                    </>
                    ...
                    <>
                        ...
                        >
                            {loading ? (
                                ...
                                <><FontAwesomeIcon icon={faRotateRight} className="mr-2" />RESEND EMAIL</>
                            )}
                        </button>
                    </>
                )}
