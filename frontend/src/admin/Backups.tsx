...
const ActionButton: React.FC<{
    ...
    onClick: () => void;
    disabled?: boolean;
}> = ({ status, label, loadingLabel = 'Processing…', color, onClick, disabled }) => (
    <button
        onClick={onClick}
    ...
    >
        {status === 'loading' ? (
            <><FontAwesomeIcon icon={faSpinner} className="animate-spin" />{loadingLabel}</>
        ) : status === 'success' ? (
...
const BackupSelector: React.FC<BackupSelectorProps> = ({ entries, value, onChange, loading, accentColor }) => (
...
const ActionCard: React.FC<ActionCardProps> = ({ icon, title, description, accentColor, children }) => (
    <div className="flex flex-col gap-4 rounded-2xl border border-app-border bg-app-card/60 p-6 shadow-[0_4px_24px_rgba(0,0,0,0.2)] backdrop-blur-sm">
        <div className="flex items-center gap-3">
            ...
            >
                <FontAwesomeIcon icon={icon} style={{ color: accentColor }} className="text-lg" />
            </div>
            <div>
...
const Backups: React.FC = () => {
    ...
    const loadEntries = useCallback(async () => {
        ...
        try {
            ...
            triggerToast(err.response?.data?.title || 'Could not load backup list', false);
        } finally {
    ...
    const finalize = (setter: (s: Status) => void, ok: boolean, reload = false) => {
        setter(ok ? 'success' : 'error');
        setTimeout(() => { setter('idle'); if (reload) loadEntries(); }, 3000);
    };
    ...
    const handleDownload = async () => {
        if (!downloadKey) { triggerToast('Please select a version', false); return; }
        setDownloadStatus('loading');
        try {
            const resp = await api.get(`/admin/backup/download/${encodeURIComponent(downloadKey)}`, { responseType: 'blob' });
    ...
    const handleRestore = async () => {
        if (!restoreKey) { triggerToast('Please select a version', false); return; }
        if (!window.confirm(`Are you sure you want to restore "${formatKey(restoreKey)}"?\nThis operation will overwrite current data.`)) return;
    ...
    const handleUpload = async () => {
        if (!uploadFile) { triggerToast('Please select a file', false); return; }
        setUploadStatus('loading');
        const formData = new FormData();
        formData.append('file', uploadFile);
        try {
            await api.post('/admin/backup/upload', formData, { headers: { 'Content-Type': 'multipart/form-data' } });
    ...
    return (
        <div className="flex flex-col gap-6">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="m-0 text-lg font-bold text-app-text">Backup Management</h2>
                ...
                >
                    <FontAwesomeIcon icon={faRefresh} className={listLoading ? 'animate-spin' : ''} />
                    Refresh list
                </button>
            </div>
                ...
                <div className="flex items-center gap-2 rounded-xl border border-app-border/50 bg-app-card/40 px-4 py-2.5 text-sm">
                    ...
                    <span className="text-app-muted">
                        {entries.length === 0
                            ...
                            : <><span className="font-bold text-app-text">{entries.length}</span> versions available in the bucket</>
                        }
                    </span>
                </div>
            )}
                ...
                <ActionCard icon={faUpload} title="Upload Backup" description="Import an external backup file to the server / R2" accentColor="#a855f7">
                    <label className="flex flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-app-border/60 bg-app-input/30 p-4 text-sm text-app-muted cursor-pointer transition-all hover:border-purple-400/40 hover:bg-purple-400/5 hover:text-app-text">
                        ...
                        {uploadFile ? (
                            ...
                            <span>Click to select a <code>.sql</code> or <code>.sql.gz.enc</code> file</span>
                        )}
                        <input
                            type="file"
