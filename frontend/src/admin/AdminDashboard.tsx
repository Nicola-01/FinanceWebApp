...
const UsersPage: React.FC<UsersPageProps> = ({ users, invites, onDeleteClick, onRevoke, onInviteCreated }) => (
    <>
...
const AdminDashboard: React.FC = () => {
    ...
    const handleRevokeInvite = async (email: string) => {
        ...
        try {
            ...
            setInvites(prev => prev.map(inv => inv.email === email ? { ...inv, status: 'REVOKED' } : inv));
    ...
    return (
        <div className="flex flex-col min-h-screen xl:h-screen xl:overflow-hidden bg-app-bg text-app-text transition-colors">
            <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
                <Sphere
                    style={{ height: '400px', width: '400px', background: '#ff2299', top: '-100px', right: '-100px', position: 'absolute' }}
                    animate={{ x: [0, 0], y: [0, 0] }}
                />
            </div>
                        ...
                        element={
                            <UsersPage
                                ...
                                onDeleteClick={(u: User) =>
                                    deleteModalRef.current?.deleteObject(u, 'user', async () => await handleConfirmDelete(u.id))
                                }
