    ...
    <div className="flex items-center gap-2 rounded-lg border border-app-green/30 bg-app-green/5 p-2 shadow-inner overflow-hidden w-full">
      ...
      {isSaving ? (
        <FontAwesomeIcon
          icon={faSpinner}
          spin
          className="text-app-green px-2"
        />
      ) : (
        <div className="flex items-center gap-1 shrink-0">
          <button
            onClick={handleSave}
            className="flex h-7 w-7 items-center justify-center text-app-green hover:bg-app-green/20 rounded-md transition-colors"
          >
            <FontAwesomeIcon icon={faCheck} />
          </button>
          <button
            onClick={onClose}
            className="flex h-7 w-7 items-center justify-center text-app-muted hover:theme-text-danger hover:theme-bg-danger-light rounded-md transition-colors"
          >
