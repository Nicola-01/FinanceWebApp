...
export const TagPickerRow: React.FC<TagPickerRowProps> = ({
  ...
  let visualClass =
    "hover:bg-app-input border theme-border-transparent cursor-pointer";
  if (isSelected) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <span
          className={`text-sm font-medium ${isSelected || isAncestor ? "text-app-text" : "text-app-muted"}`}
        >
          {tag.name}{" "}
          {isParentHeader && (
            <span className="opacity-50 text-xs italic ml-1">(General)</span>
          )}
        </span>
      </div>
