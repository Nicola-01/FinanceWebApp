...
export const RecurringPaymentToggle: React.FC<Props> = ({
  isRecurring,
  setIsRecurring,
}) => {
  return (
    <div className="rounded-xl border border-app-border bg-app-input p-4 transition-all">
      ...
      >
        <div className="flex items-center gap-3">
          <div
            className={`flex h-10 w-10 items-center justify-center rounded-lg ${isRecurring ? "bg-app-green/20 text-app-green" : "bg-app-input text-app-muted"}`}
          >
      ...
      {isRecurring && (
        <div className="mt-4 rounded-lg border border-dashed border-[var(--color-app-sky)]/30 bg-[var(--color-app-sky)]/10 p-4 text-center animate-[fadeIn_0.2s_ease-out]">
          <span className="text-sm font-bold text-app-sky">
            🚀 Recurring Payments feature is coming soon!
          </span>
        </div>
      )}
    </div>
