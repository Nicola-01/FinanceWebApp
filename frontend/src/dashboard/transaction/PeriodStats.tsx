...
export const PeriodStats: React.FC<PeriodStatsProps> = ({
  ...
  const tagStats = transactions.reduce(
    (acc, tx) => {
      if (!acc[tx.tag.name]) {
        acc[tx.tag.name] = {
          income: 0,
    ...
    {} as Record<
      string,
      { income: number; expense: number; color: string; icon: string }
    >,
  );
      ...
      <div className="flex items-center justify-between">
        ...
        {!isLoading && transactions.length > 0 && (
          ...
          >
            ...
            <FontAwesomeIcon
              icon={showDistribution ? faChevronUp : faChevronDown}
            />
          </button>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 items-start">
        <div className="theme-bg-overlay-light border border-app-border rounded-2xl p-4 flex flex-col transition-all">
          <div className="text-center">
            <p className="text-app-muted text-xs font-bold uppercase tracking-widest mb-1">
              Period Income
            </p>
            {isLoading ? (
              <SkeletonAmount />
            ) : (
              <p className="text-app-green font-bold font-app-mono text-xl">
          ...
          {!isLoading && showDistribution && incomeTags.length > 0 && (
            <div className="mt-4 pt-4 border-t border-app-border flex flex-col gap-3 animate-[fadeIn_0.2s_ease-out]">
              {incomeTags.map(([tagName, stats]) => {
                const percentage =
                  totals.income > 0 ? (stats.income / totals.income) * 100 : 0;
                return (
                  <div key={tagName} className="flex flex-col gap-1.5">
                    <div className="flex justify-between items-center text-xs">
                      ...
                      >
                        <FontAwesomeIcon
                          icon={ICONS[stats.icon as IconKey] || "tag"}
                        />
                      ...
                      <div className="flex items-center gap-2 font-app-mono theme-text-muted">
                        ...
                        <span className="theme-text-subtle text-[10px] w-8 text-right">
        ...
        <div className="theme-bg-overlay-light border border-app-border rounded-2xl p-4 flex flex-col transition-all">
          <div className="text-center">
            ...
            {isLoading ? (
              ...
              <p className="text-app-red font-bold font-app-mono text-xl">
          ...
          {!isLoading && showDistribution && expenseTags.length > 0 && (
            <div className="mt-4 pt-4 border-t border-app-border flex flex-col gap-3 animate-[fadeIn_0.2s_ease-out]">
              {expenseTags.map(([tagName, stats]) => {
                const percentage =
                  totals.expense > 0
                    ? (stats.expense / totals.expense) * 100
                    : 0;
                return (
                  <div key={tagName} className="flex flex-col gap-1.5">
                    <div className="flex justify-between items-center text-xs">
        ...
        <div className="bg-app-input border sm:col-span-2 lg:col-auto border-app-border rounded-2xl p-4 flex flex-col text-center justify-center min-h-19">
          ...
          {isLoading ? (
            ...
            <p
              className={`font-bold font-app-mono text-xl ${netTotal >= 0 ? "text-app-green" : "text-app-red"}`}
            >
              {netTotal >= 0 ? "+" : ""}
              {netTotal.toFixed(2)}
            </p>
