...
export const TagsTab: React.FC<TagsTabProps> = ({ tags, onAddTag, onUpdateTag, onDeleteTag }) => {
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            {savingMain ? <FontAwesomeIcon icon={faSpinner} spin className="text-[#00ff7f] ml-auto"/> : (
                                <div className="flex items-center gap-1 ml-auto">
                                    ...
                                    <button onClick={() => { setIsAddingTag(false); setNewTag({...newTag, name: ''}) }} className="flex h-8 w-8 items-center justify-center rounded-md bg-white/5 text-white/40 hover:bg-red-500/20 hover:text-red-500 transition-colors" title="Cancel"><FontAwesomeIcon icon={faXmark}/></button>
                                </div>
                            )}
                        </div>
                    </div>
                ) : (
                    <button onClick={() => setIsAddingTag(true)} className="cursor-pointer group flex items-center justify-center gap-4 p-4 rounded-2xl border border-dashed border-white/30 bg-white/5 backdrop-blur-md transition-all hover:bg-white/10 hover:border-white/50 text-left min-h-[100px]">
