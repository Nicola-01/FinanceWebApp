...
export const TagsTab: React.FC = () => {
  ...
  tags.forEach((tag) => {
    if (!tag.parentName)
      organizedTags[tag.name] = { parent: tag, children: [] };
  });
  tags.forEach((tag) => {
    if (tag.parentName && organizedTags[tag.parentName])
      organizedTags[tag.parentName].children.push(tag);
    else if (tag.parentName && !organizedTags[tag.parentName])
      organizedTags[tag.parentName] = {
        parent: tag,
        children: [],
      };
  });
          ...
          <div className="rounded-2xl border border-app-border bg-app-card/20 p-6 backdrop-blur-md animate-[fadeIn_0.2s_ease-out]">
            ...
            {!isLoading && wallet.userRole !== "VIEWER" && (
              <div className="mb-8 w-full">
                {isAddingTag ? (
                  <div className="group flex w-full items-center gap-4 rounded-2xl border-2 border-app-green/30 bg-app-green/5 p-4 text-app-text transition-all shadow-sm">
