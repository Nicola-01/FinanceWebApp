...
const TagCard: React.FC<TagCardProps> = ({
  ...
  const handleSaveParentName = async () => {
    if (parentNameVal.trim() && parentNameVal !== parent.name) {
      const success = await onUpdateTag(parent.name, {
        ...parent,
        name: parentNameVal.trim(),
      });
        ...
        <div className="flex-1 min-w-0">
          {editingParentName ? (
            <div className="flex items-center gap-2">
              ...
              />
              <button
                onClick={handleSaveParentName}
                className="text-app-green hover:text-app-text transition-colors"
              >
              ...
              <button
                ...
                className="text-app-muted hover:theme-text-danger transition-colors"
              >
            ...
            <div className="flex items-center justify-between">
              <div className="min-w-0">
                <h1 className="font-bold text-app-text text-2xl truncate">
                  {parent.name}
                </h1>
              </div>
                ...
                {wallet.userRole !== "VIEWER" && (
                  <>
                    <button
                      ...
                      className="text-app-muted/40 hover:theme-text-warning transition-colors"
                    >
                      <FontAwesomeIcon
                        icon={faPenToSquare}
                        className="text-sm"
                    ...
                    <button
                      ...
                      className="text-app-muted/40 hover:theme-text-danger transition-colors"
                    >
      ...
      <div className="mt-2 flex flex-col gap-2 pl-4 border-l-2 border-app-border">
        {isAddingChild ? (
          <div className="flex items-center gap-2 rounded-lg p-2 bg-app-card/40 border border-app-green/30">
            <FontAwesomeIcon
              icon={faArrowTurnUp}
              className="rotate-90 text-app-green text-xs shrink-0"
            />
            <input
              autoFocus
            ...
            {loading ? (
              <FontAwesomeIcon
                icon={faSpinner}
                spin
                className="text-app-green text-xs shrink-0"
              />
            ) : (
              <>
                <button
                  onClick={handleAddChild}
                  className="text-app-green hover:text-app-text transition-colors shrink-0"
                >
                  <FontAwesomeIcon icon={faCheck} />
                </button>
                <button
                  onClick={() => setIsAddingChild(false)}
                  className="text-app-muted hover:theme-text-danger transition-colors shrink-0"
                >
                  <FontAwesomeIcon icon={faXmark} />
                </button>
              </>
          ...
          wallet.userRole !== "VIEWER" && (
            <button
              ...
              className="flex items-center gap-3 rounded-lg p-2 transition-colors hover:bg-app-input text-left group cursor-pointer"
            >
              <FontAwesomeIcon
                icon={faPlus}
                className="text-app-muted/20 group-hover:text-app-green text-xs transition-colors"
              />
              <span className="text-sm font-medium text-app-muted group-hover:text-app-green transition-colors">
                Add sub-category
              </span>
            </button>
          )
