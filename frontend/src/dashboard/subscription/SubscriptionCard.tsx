...
const formatCompactFrequency = (interval: number, type: string) => {
    ...
    switch (type.toUpperCase()) {
        case 'MONTHLY': return `${i}mo`;
        case 'YEARLY': return `${i}yr`;
        case 'WEEKLY': return `${i}wk`;
        case 'DAILY': return `${i}d`;
        default: return `${i}${type.toLowerCase().replace('ly', '')}`; // Fallback di sicurezza
    }
};
...
export const SubscriptionCard: React.FC<SubscriptionCardProps> = ({subscription, date, onClick}) => {
    ...
    return (
        ...
        >
            <div className="flex items-center gap-3 sm:gap-4 min-w-0 shrink max-w-[65%] lg:max-w-[75%]">
                ...
                <div className="flex flex-col md:flex-row md:items-center items-start gap-1.5 md:gap-3 min-w-0 py-0.5">
                    ...
                    <div className="flex items-center gap-1.5 overflow-hidden shrink-0 flex-wrap">
                        {subscription.tag && <TagBadge tag={subscription.tag}/>}

                        {/* Mostriamo un badge testuale per gli abbonamenti non attivi */}
                        {subscription.status === 'PAUSED' && (
                            <span className="px-2 py-0.5 rounded-md bg-amber-500/10 text-amber-500 text-[10px] font-bold uppercase tracking-wider border border-amber-500/20">Paused</span>
                        )}
                        {subscription.status === 'COMPLETED' && (
                            <span className="px-2 py-0.5 rounded-md bg-white/10 text-white/50 text-[10px] font-bold uppercase tracking-wider border border-white/10">Completed</span>
                        )}
                    </div>
                </div>
            </div>
            ...
            <div className="flex flex-col items-end justify-center shrink-0 pl-3 min-w-[100px]">

                {/* Riga 1: Importo + Valuta + Frequenza (es: -12.99 € / mo) */}
                <div className={`text-right text-lg font-bold font-app-mono inline-flex items-baseline justify-end gap-1 ${isIncome ? 'text-[#00ff7f]' : 'text-[#ff4d4d]'}`}>
                ...
                {subscription.status === 'ACTIVE' && daysLeft !== null && (
                    <span className={`text-xs font-medium mt-0.5 transition-colors duration-500 ${getDaysLeftColor(daysLeft, isIncome)}`}>
